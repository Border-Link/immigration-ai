"""
Permission classes for AI Reasoning Log access control.

These permissions are specific to the ai_decisions module and provide
clear, self-documenting access control for AI reasoning logs.
"""
...
class CanViewAIReasoningLog(permissions.BasePermission):
    """
    Permission to view AI reasoning logs.
    
    Rules:
    - User is reviewer (role='reviewer' AND is_staff/is_superuser)
    - User is admin/staff (is_staff OR is_superuser)
    
    AI reasoning logs contain sensitive information (prompts, responses, tokens)
    and are restricted to reviewers and admins for auditing and debugging purposes.
    ...
    def has_object_permission(self, request, view, obj):
        """
        Check if user has permission to view the AI reasoning log.
        
        For now, if user has permission at the view level, they can view any log.
